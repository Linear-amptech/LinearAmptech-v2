            ...
            <section key={group.id}>
              <Link
                ...
                className={`flex items-center justify-between rounded-2xl border px-4 py-3 transition-colors ${
                  isActiveGroup
                    ? "border-[color:var(--color-primary-deep)] bg-[color:var(--color-surface-soft)] text-[color:var(--color-text)]"
                ...
                <div className="mt-4 space-y-4">
                  {group.panels.map((panel) => (
                    <div key={panel.title}>
                      <Link
                        href={panel.href}
                        className="text-sm font-semibold text-[color:var(--color-primary-deep)] transition-colors hover:text-[color:var(--color-primary)]"
                      >
                        {panel.title}
                      </Link>
